...
export async function validatePlugin(
  ...
  const pluginJson = readFileFromArchive(
    contents,
  ...
  if (
    ...
    errors.push({
      code: 'NO_COMPONENTS',
      message:
        'Plugin must contain at least one component: skills/, commands/, agents/, hooks/, .mcp.json, or .lsp.json',
    });
  }
  ...
  const metadata: ExtractedMetadata = {
    name: (pluginConfig?.name as string) || '',
    description: (pluginConfig?.description as string) || '',
    version: (pluginConfig?.version as string) || '1.0.0',
...
function findSkillDirs(contents: ArchiveContents): string[] {
  ...
  for (const [filePath, buffer] of contents.files.entries()) {
    ...
    if (filePath.startsWith(prefix)) {
      ...
      relativePath = filePath.slice(
        `${contents.rootDir}/${prefix}`.length,
      );
    }
    if (relativePath) {
